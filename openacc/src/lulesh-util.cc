                               ...
                               Int_t numRanks)
{
  ...
  // Compute checksum of pressures and positions. Energies are not included
  // because they dwarf the other values.
  Real_t chksm = 0;
  for (Index_t i=0; i<nx; ++i) {
    //chksm += locDom.e(i);
    chksm += locDom.p(i);
    chksm += locDom.x(i) + locDom.y(i) + locDom.z(i);
  }
  printf("checksum: %12.6e\n\n", chksm);
